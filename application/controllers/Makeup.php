...
class Makeup extends CI_Controller
{
	public function __construct() 
	{
		...
		$this->load->model(array('Makeup_model'));
	...
	{	
        ...
        $data['title'] = "新秘造型-相簿管理";

		//相簿data
        $query = $this->Makeup_model->get_all_makeup_catrgory();
        if (!empty($query)) {
            $data['photo'] = $query;
        }
		...
		$this->load->view('makeup/index');
		$this->load->view('templates/footer');
		$this->load->view('makeup/app');
    }
    //改相簿狀態
    public function ajax_change_status_category(){
        ...
        $res = $this->Makeup_model->update_status_category($category_id,$status);
        if ($res === TRUE) {
            $text = '修改成功';
            $code = 0;
        }else {
    ...
    public function add_category()
    {
		...
		$data['title'] = "新秘造型-新增相簿";

		$this->load->view('templates/head',$data);
		$this->load->view('templates/header');
		$this->load->view('templates/title');
		$this->load->view('makeup/add_category');
		$this->load->view('templates/footer');
		$this->load->view('makeup/upload_img_app');
    }
    //新增相簿表單
        ...
        if (!$berror) {
            foreach ($res as $row) {
                if ($row['code'] == 0) {
                    ...
                    $insert_res = $this->Makeup_model->insert_new_category($category_name,$dir);
                    if ($insert_res === FALSE) {
                        $msg .= "新增相簿失敗";
                        //刪除圖片
                        if (!empty($dir)) {
    ...
    public function form_edit_category()
    {
        ...
        if (empty($_FILES['userfile']['name'][0])) { //只更新資料
            $update_res = $this->Makeup_model->update_category_img(NULL,$cateogry_id,$category_name,0);
            if ($update_res === FALSE) {
                $msg .= "更新失敗";
            }else{
                $msg .= "更新成功";
            ...
            if (!$berror) {
                foreach ($res as $row) {
                    if ($row['code'] == 0) {
                        $dir = trim($row['dir']);
                        //找原圖
                        $query = $this->Makeup_model->get_category_img($cateogry_id);
                        $original_img = !empty($query) ? $query['default_img'] : NULL;
                        $update_res = $this->Makeup_model->update_category_img($dir,$cateogry_id,$category_name,1);
                        if ($update_res === FALSE) {
                            $msg .= "更新失敗";
                            //刪除圖片
                            if (!empty($dir)) {
        ...
        $data['title'] = "結果";
        $data['msg'] = $msg;
        $data['url'] = !empty($this->agent->referrer())? $this->agent->referrer() : 'makeup';
    ...
    public function ajax_edit_category_seq()
    {
        ...
        //排序
        if (!$berror) {
            $res = $this->Makeup_model->edit_category_seq($sort_array);
            //刪資料庫
            if ($res === FALSE) {
                $msg = "調整順序失敗";
                $code = 3;
    ...
    public function ajax_del_category()
    {
        ...
        //刪除
        if (!$berror) {
            $query = $this->Makeup_model->get_del_category_data($id_array);
            if (!empty($query)) {
                //刪相簿資料&相簿圖片
                if (!empty($query['category_img'])) {
                    foreach ($query['category_img'] as $row) {
                        $img = $row['default_img'];
                        $id = $row['id'];
                        $res = $this->Makeup_model->del_category($id);
                        //刪資料庫
                        if ($res === FALSE) {
                ...
                if (!empty($query['img'])) {
                    foreach ($query['img'] as $row) {
                        $img = $row['img'];
                        $id = $row['id'];
                        $res = $this->Makeup_model->del_img($id);
                        //刪資料庫
                        if ($res === FALSE) {
    ...
    //相簿裡面的相片
    public function makeup_category_view($id = NULL)
    {
        if (intval($id) > 0) {
        ...
        //相簿裡的相片
        $query = $this->Makeup_model->get_makeup_catrgory_img($cateogry_id);
        if (!empty($query)) {
            $data['photo'] = $query['photo'];
            $data['category'] = $query['category'];
        }

        $data['title'] = '新秘造型-'.$query['category']['name'];

        $this->load->view('templates/head',$data);
        $this->load->view('templates/header');
        $this->load->view('templates/title');
        $this->load->view('makeup/makeup_category_view');
        $this->load->view('templates/footer');
        $this->load->view('makeup/makeup_category_view_app');
    }
    //改相片狀態
    public function ajax_change_status_makeup(){
        //限定POST,防機器人,來源
        if ($this->input->server('REQUEST_METHOD') != 'POST' || $this->agent->is_robot() || $this->agent->is_referral()) {
            show_error('Access forbidden', 403);
            exit;
        ...
        //makeup_id
		$this->form_validation->set_rules(
            'makeup_id', 'makeup_id','trim|required|greater_than_equal_to[1]|less_than_equal_to[999999999]',
            array(
                'required'  => '非法提交參數',
                'greater_than_equal_to' => '非法提交參數',
                'less_than_equal_to' => '非法提交參數'
        ...
        $status = $post_data['status'];
        $category_id = $post_data['category_id'];
        $makeup_id = $post_data['makeup_id'];
        $res = $this->Makeup_model->update_status_makeup($category_id,$makeup_id,$status);
        if ($res === TRUE) {
            $text = '修改成功';
            $code = 0;
        }else {
	...
	public function upload_img($cid = NULL)
	{
        ...
        $data['title'] = "新秘造型-上傳圖片";

        if (intval($cid) > 0) {
            $query = $this->Makeup_model->get_makeup_catrgory_name($cid);
            if (!empty($query)) {
            ...
            $this->load->view('templates/header');
            $this->load->view('templates/title');
            $this->load->view('makeup/upload_img');
            $this->load->view('templates/footer');
            $this->load->view('makeup/upload_img_app');
        }else {
            //html
    ...
    public function ajax_del_img()
    {
        ...
        //刪除
        if (!$berror) {
            $query = $this->Makeup_model->get_img($id_array);
            if (!empty($query)) {
    ...
    public function form_upload_img()
    {
        ...
        if (!$berror) {
            ...
            foreach ($res as $row) {
                if ($row['code'] == 0) {
                    $dir = trim($row['dir']);
                    $update_res = $this->Makeup_model->insert_new_img($dir,$cateogry);
                    if ($update_res === FALSE) {
                        $msg .= "新增相片失敗 : $dir";
                        //刪除圖片
                        if (!empty($dir)) {
        ...
        $data['title'] = "上傳結果";
        $data['msg'] = $msg;
        $data['url'] = !empty($post_data['url'])? $post_data['url'] : 'makeup';
    ...
    public function ajax_edit_img_seq()
    {
        ...
        //排序
        if (!$berror) {
            $res = $this->Makeup_model->edit_img_seq($sort_array);
            if ($res === FALSE) {
                $msg = "調整順序失敗";
                $code = 3;
                $berror = TRUE;
